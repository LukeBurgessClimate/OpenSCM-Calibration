...
```{code-cell} ipython3
model_runner.run_model([1e-10, 1e-10, 1e-13]).timeseries()
```
...
```{code-cell} ipython3
cost_calculator.calculate_cost(model_runner.run_model([2e-10, 2e-10, 2e-13]))
```
...
max_n_runs = len(parameters) + 10 * maxiter
