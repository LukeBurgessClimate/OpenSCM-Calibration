...
concentrations=add_co_concentrations(concentrations,noaa_global,start=1992,end=2023)
...
about 100 TgCO yr-1 once acounting for anthrophenic and oxidation of ch4.
...
```{code-cell} ipython3
def insert_rigby(scen,rigby_scen):
    out=scen.copy().timeseries()
    start=rigby_scen["time"].iloc[0]
    stop=rigby_scen["time"].iloc[-1]
    out.loc[out.index.get_level_values("variable").isin(["Atmospheric Concentrations|OH"]),start:stop]=rigby_scen.values
    return scmdata.ScmRun(out)
...
oh_concentrations= scmdata.ScmRun(
    pd.DataFrame(
        2.9e-5 * np.zeros(years.shape)[np.newaxis, :],
        index=pd.MultiIndex.from_arrays(
            [
                [ "Atmospheric Concentrations|OH"],
                ["ppb"],
...
oh_concentrations = insert_rigby(oh_concentrations,rigby)
...
def set_past_co_nan(scen,years):
    out=scen.copy().timeseries()
    out.loc[out.index.get_level_values("variable").isin(["Atmospheric Concentrations|CO"]),:dt.datetime(years[0]-1,1,1)]=0
    return scmdata.ScmRun(out)

# target=set_past_co_nan(target,years_co)

# for vdf in target.filter(year=range(1990,1993)).groupby("variable"):
